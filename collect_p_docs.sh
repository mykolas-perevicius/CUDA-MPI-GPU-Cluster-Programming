...
# This script collects key project documentation files (README, AI context, Research, Discussion)
...
# Ordered logically: README, AI context, then Research, then Discussion Log
INPUT_FILES=(
    "README.md"         # Added general project README
    "AI.md"
    "RESEARCH.md"       # Added RESEARCH.md
    "DISCUSSION.md"
)
...
echo "" >> "${OUTPUT_FILE}"
echo "*This file combines key project README, context, research, and discussion logs.*" >> "${OUTPUT_FILE}" # Updated description
...
for filename in "${INPUT_FILES[@]}"; do
    ...
    if [[ -f "$full_path" ]]; then
        ...
        # Use sed to add a "> " prefix for blockquoting ai_context.txt if desired (Kept for potential future use)
        if [[ "$filename" == "ai_context.txt" ]]; then # Example: Use filename base if needed
