    ...
    int H, int W, int C)
{
    ...
    nextC = paramsConv1.K;
    // Calculate required size using size_t
    size_t required_size_conv1 = static_cast<size_t>(nextH) * nextW * nextC;
    if (current_output->size() < required_size_conv1) {
        current_output->resize(required_size_conv1);
    }
    ...
    nextC = currentC;
    size_t required_size_pool1 = static_cast<size_t>(nextH) * nextW * nextC;
     // Ensure the non-active buffer is large enough or resize
    if (current_output->size() < required_size_pool1) {
        current_output->resize(required_size_pool1);
    }
    ...
    nextC = paramsConv2.K;
    size_t required_size_conv2 = static_cast<size_t>(nextH) * nextW * nextC;
     // Ensure the non-active buffer is large enough or resize
    if (current_output->size() < required_size_conv2) {
        current_output->resize(required_size_conv2);
    }
    ...
    nextC = currentC;
    size_t required_size_pool2 = static_cast<size_t>(nextH) * nextW * nextC;
     // Ensure the non-active buffer is large enough or resize
    if (current_output->size() < required_size_pool2) {
        current_output->resize(required_size_pool2);
    }
    serialMaxPoolLayer(*current_output, *current_input,
    ...
    size_t required_size_lrn2 = static_cast<size_t>(nextH) * nextW * nextC;
    // Ensure the non-active buffer is large enough or resize
    if (current_output->size() < required_size_lrn2) {
        current_output->resize(required_size_lrn2);
    }
    ...
    size_t num_to_print = std::min(static_cast<size_t>(10), input_data.size()); // Use size_t
    for(size_t i = 0; i < num_to_print; ++i) { // Use size_t
