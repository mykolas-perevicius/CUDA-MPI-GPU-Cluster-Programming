...
// CUDA?     cudaError_t err = call; \
// CUDA?     if (err != cudaSuccess) { \
...
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_input_batch, input_bytes));
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_pool1_out, pool1_out_bytes));
...
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_conv2_out, conv2_out_bytes));
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_pool2_out, pool2_out_bytes));
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_lrn2_out,  lrn2_out_bytes));
...
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_conv2_weights, conv2_weights_bytes));
// CUDA?     CUDA_CHECK(cudaMalloc((void**)&d_conv2_biases, conv2_biases_bytes));
    // (In real implementation, weights are copied from host via MPI_Bcast.)

    // Create a CUDA stream (optional)
// CUDA? // CUDA?     cudaStream_t stream = 0;

    // Start timing the Block2 operations
// CUDA? // CUDA?     cudaEvent_t start, stop;
// CUDA?     CUDA_CHECK(cudaEventCreate(&start));
// CUDA?     CUDA_CHECK(cudaEventCreate(&stop));
// CUDA?     CUDA_CHECK(cudaEventRecord(start, stream));
...
// CUDA?     CUDA_CHECK(cudaEventRecord(stop, stream));
// CUDA?     CUDA_CHECK(cudaEventSynchronize(stop));
    float kernel_time_ms = 0.f;
// CUDA?     CUDA_CHECK(cudaEventElapsedTime(&kernel_time_ms, start, stop));
...
// CUDA? // CUDA?     CUDA_CHECK(cudaMemcpy(h_output_batch, d_lrn2_out, final_output_bytes, cudaMemcpyDeviceToHost));

    // Free device memory
// CUDA?     CUDA_CHECK(cudaFree(d_input_batch));
// CUDA?     CUDA_CHECK(cudaFree(d_pool1_out));
// CUDA?     CUDA_CHECK(cudaFree(d_conv2_out));
// CUDA?     CUDA_CHECK(cudaFree(d_pool2_out));
// CUDA?     CUDA_CHECK(cudaFree(d_lrn2_out));
// CUDA?     CUDA_CHECK(cudaFree(d_conv2_weights));
// CUDA?     CUDA_CHECK(cudaFree(d_conv2_biases));

// CUDA?     CUDA_CHECK(cudaEventDestroy(start));
// CUDA?     CUDA_CHECK(cudaEventDestroy(stop));
}
