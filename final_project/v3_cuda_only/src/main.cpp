...
// MPI?     MPI_Bcast(h_conv1_weights.data(), conv1_weights_count, MPI_FLOAT, 0, MPI_COMM_WORLD);
// MPI?     MPI_Bcast(h_conv1_biases.data(),  conv1_biases_count,  MPI_FLOAT, 0, MPI_COMM_WORLD);
// MPI?     MPI_Bcast(h_conv2_weights.data(), conv2_weights_count, MPI_FLOAT, 0, MPI_COMM_WORLD);
// MPI?     MPI_Bcast(h_conv2_biases.data(),  conv2_biases_count,  MPI_FLOAT, 0, MPI_COMM_WORLD);

    // ----- Run the full AlexNet forward pass (Block1 and Block2) -----
// MPI?     double start_t = MPI_Wtime();
    ...
    alexnetForward(h_inputData.data(), h_localOutput.data(),
                   ...
                   outputSize_per_rank);
// MPI?     double end_t = MPI_Wtime();
    double elapsed = end_t - start_t;
// MPI?     std::cout << "Process " << rank << " forward pass time (MPI_Wtime): " 
...
// MPI?     MPI_Gather(h_localOutput.data(), outputSize_per_rank, MPI_FLOAT,
// MPI?                gatheredOutputs.data(), outputSize_per_rank, MPI_FLOAT,
